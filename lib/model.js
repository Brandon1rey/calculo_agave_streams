'use strict';
// Motor de cálculo y capa de datos (sin dependencias). SIN datos mock.
// Cubre: recepción de materia prima (pesaje, inspección, ticket), producción
// E2E (órdenes con control de rendimiento por etapa) y una calculadora
// fine-tunable (cascada de rendimiento por etapas, presets, merma por causa).
const fs = require('fs');
...
function emptyDb(){
  return { version: 4, meta: { hoy: todayStr() }, parametrosCalculo: JSON.parse(JSON.stringify(DEFAULT_CALC)),
...
class Store {
  ...
  load(){
    try {
      ...
      if (p && p.version >= 3 && Array.isArray(p.streams) && Array.isArray(p.camiones)){
        p.version = 4;
        p.meta = p.meta || {}; p.meta.hoy = todayStr();
        p.parametrosCalculo = Object.assign({}, JSON.parse(JSON.stringify(DEFAULT_CALC)), p.parametrosCalculo || {});
        p.razonesSociales = (p.razonesSociales || []).map(r => Object.assign({}, r, { parametros: Object.assign({}, PRESETS.base, r.parametros || {}) }));
        p.ordenes = p.ordenes || [];
        this.data = p;
        return this.data;
      }
    } catch (e) { /* ausente o inválido -> arranca vacío */ }
    this.data = emptyDb();
    this.save();
    return this.data;
  }
  ...
  reset(){ this.data = emptyDb(); this.save(); return this.data; }
}
...
function updateParametrosCalculo(db, body){
  ...
  if (body.mermaCausas !== undefined){
    ...
    const claves = ['hojas', 'danado', 'fibra', 'cortes', 'otros'];
    const out = {};
    let suma = 0;
    claves.forEach(c => { const v = parseNum(mc[c]); if (!(v >= 0 && v <= 100)) throw err('mermaCausas.' + c + ' debe estar entre 0 y 100'); out[c] = v; suma += v; });
...
function applyPreset(db, id, nombre){
  ...
  const r = db.razonesSociales.find(x => x.id === id);
  if (!r) throw err('Razón social no encontrada', 404);
  r.parametros = JSON.parse(JSON.stringify(PRESETS[nombre]));
  r.preset = nombre;
  return db;
}
...
function addStream(db, body){
  ...
  const mermaRate = body.mermaRate === undefined ? 0.10 : parseNum(body.mermaRate); // default 10% (generalización: el detalle fino es opcional)
  if (!nombre) throw err('nombre es requerido');
...
function createBackup(db){
  ...
  const ts = new Date();
  const id = 'bkp-' + ts.getFullYear() + pad(ts.getMonth() + 1) + pad(ts.getDate()) + '-' + pad(ts.getHours()) + pad(ts.getMinutes()) + pad(ts.getSeconds()) + '-' + Math.random().toString(36).slice(2, 6);
  const file = path.join(BACKUP_DIR, id + '.json');
  fs.writeFileSync(file, JSON.stringify({ id, createdAt: ts.toISOString(), data: db }, null, 2), 'utf8');
  return { id, createdAt: ts.toISOString() };
}
function listBackups(){
  ...
  return fs.readdirSync(BACKUP_DIR).filter(f => f.endsWith('.json')).map(f => {
    try {
      ...
      return { id: b.id || f.replace('.json', ''), createdAt: b.createdAt || '', file: f,
        counts: { razones: (b.data.razonesSociales || []).length, streams: (b.data.streams || []).length, camiones: (b.data.camiones || []).length, consumos: (b.data.consumos || []).length, ordenes: (b.data.ordenes || []).length } };
    } catch (e){ return { id: f.replace('.json', ''), createdAt: '', file: f, counts: {} }; }
  }).sort((a, b) => (a.createdAt < b.createdAt ? 1 : -1));
}
function restoreBackup(id){
  if (!safeBackupId(id)) throw err('id de respaldo inválido', 400);
  const file = path.join(BACKUP_DIR, id + '.json');
  if (!fs.existsSync(file)) throw err('Respaldo no encontrado', 404);
  let b;
  try { b = JSON.parse(fs.readFileSync(file, 'utf8')); } catch (e){ throw err('Respaldo ilegible', 400); }
  if (!b || !b.data || !Array.isArray(b.data.streams)) throw err('Respaldo inválido', 400);
  b.data.version = 4;
  b.data.meta = { hoy: todayStr() };
  b.data.parametrosCalculo = Object.assign({}, JSON.parse(JSON.stringify(DEFAULT_CALC)), b.data.parametrosCalculo || {});
  b.data.ordenes = b.data.ordenes || [];
  return b.data;
}
...
module.exports = {
  Store, emptyDb, computeState, PRESETS, DEFAULT_CALC, CLAVES_ETAPA,
  cascadeLPerTon, cascadeBreakdown, validarParametros,
  updateParametrosCalculo,
  addRazonSocial, updateRazonSocial, deleteRazonSocial, applyPreset,
  addStream, updateStream, deleteStream,
  addCamion, updateCamion, deleteCamion, recibirCamion,
  addConsumo, deleteConsumo,
  addOrden, updateOrden, deleteOrden,
  todayStr, DATA_DIR, DB_FILE, BACKUP_DIR,
